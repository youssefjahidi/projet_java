...
public class UberPool extends Rides {
	...
	public UberPool(Customer customer, GPSPosition destination, int idAct){
		super(customer, destination, uberPool, "UberPool" + (String) Integer.toString(idAct));
	...
	public void request() {
		for(Standard standard: Standard.S_exist){
			...
			if( distancechauffeur<5 && this.nbpersonne<= standard.getnbSeats() ){
